ФУНКЦИОНАЛЬНОСТЬ
Скрипт настраивается и запускается при разных параметрах. 
На данный момент на git загружен скрипт при запуске которого выводятся наиболее релевантные товары. 
---
